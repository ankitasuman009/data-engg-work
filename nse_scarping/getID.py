...
def getId(company):
    ...
    details = 'https://www.nseindia.com/api/quote-equity?symbol={}'
    session.get('https://www.nseindia.com/', headers=head)

    search_results = session.get(url=url.format(company), headers=head)
    # print(search_results.json())
    search_result = search_results.json()['symbols'][0]['symbol']
    # print("===================================================================")
    company_details = session.get(url=details.format(search_result), headers=head)
